...
//  test_glMatrix();	// make sure that the fast vector/matrix library we use
  ...
  drawAll();
  //---------OPTIONAL:  make our initial ray-traced image automatically:
  // (do everything the 't' key does...)
/*
...
//=============================================================================
  ...
  switch (g_SceneNum) {
    case 0:
      var xRad = -gui.camYawInit + Math.PI;
      var yRad = -gui.camPitchInit - 0.2;
      vec4.set(gui.camEyePt, 37, 0, 7,1);
      gui.camAim(xRad, yRad);
      break;
    case 1:
      var xRad = -gui.camYawInit + (-2.033);
      var yRad = -gui.camPitchInit - 0.1;
      vec4.set(gui.camEyePt, 5.4, 0.4, 3,1);
      gui.camAim(xRad, yRad);
      break;
    case 2:
      var xRad = -gui.camYawInit + 2.7;
      var yRad = -gui.camPitchInit - 0.1;
      vec4.set(gui.camEyePt, 8, 6, 3,1);
      gui.camAim(xRad, yRad);
      break;
    case 3:
      var xRad = -gui.camYawInit + 7*Math.PI/8;
      var yRad = -gui.camPitchInit - 0.1;
      vec4.set(gui.camEyePt, 7, -3, 3,1);
      gui.camAim(xRad, yRad);
      break;
    case 4:
      var xRad = -gui.camYawInit + 2*Math.PI/4;
      var yRad = -gui.camPitchInit - 0.1;
      vec4.set(gui.camEyePt, 10, 0, 4,1);
      gui.camAim(xRad, yRad) 
      break;
    case 5:
      var xRad = -gui.camYawInit - 2*Math.PI/4;
...
function lampToggle(k) {

  var state = "";
  var colr = "";

  if (g_myScene.lamp[k].on) {
    g_myScene.lamp[k].on = false;
    state = "OFF";
  }
  else {
    g_myScene.lamp[k].on = true;
    state = "ON";
  }
  ...
  document.getElementById('lamp' + k).innerHTML = 'Lamp ' + (k+1) + ': ' + colr + ' @ (' + g_myScene.lamp[k].pos[0] 
                                                                         + ', ' + g_myScene.lamp[k].pos[1] 
                                                                         + ', ' + g_myScene.lamp[k].pos[2] + '), ' + state;
...
function updateUI() {
	...
	// boid center
	lampX = document.getElementById('x_slider').value;
	lampY = document.getElementById('y_slider').value;
	lampZ = document.getElementById('z_slider').value;

  var state = "";

  if (g_myScene.lamp[3].on) {
    state = "ON";
  }
  else {
    state = "OFF"
	...
	document.getElementById('lamp3').innerHTML = 'Lamp 4: white @ (' + lampX + ', ' + lampY + ', ' + lampZ + '), ' + state;
  
  vec4.set(g_myScene.lamp[3].pos, lampX, lampY, lampZ, 1.0);

  g_myScene.recursion = parseInt(document.getElementById('recursion').value);
